...
async function googleDriveToken() {
  ...
  if (googleDriveOAuthEnabled) {
    ...
    if (!response.ok || !result.access_token) { const code = result?.error || ""; const detail = result?.error_description || ""; throw new Error(`Google Drive authentication failed (${response.status})${code ? `: ${String(code).slice(0, 120)}` : ""}${detail ? ` — ${String(detail).slice(0, 300)}` : ""}`); }
